...
const updateEvent = async (req, resp) => {
  try {
    let { eventName, updatedBody } = req.body;
    let response = await EventModel.updateOne(
      { eventName: eventName },
      {
        $set: updatedBody,
      }
...
const updatePreEvent = async (req, resp) => {
  try {
    let { eventName, updatedBody } = req.body;
    let response = await PreEventModel.updateOne(
      { eventName: eventName },
      {
        $set: updatedBody,
      }
...
const updateGuestTalks = async (req, resp) => {
  try {
    let { eventName, updatedBody } = req.body;
    let response = await GuestTalksModel.updateOne(
      { eventName: eventName },
      {
        $set: updatedBody,
      }
...
const deleteEvent = async (req, resp) => {
  try {
    let response = await EventModel.findOneAndDelete({
